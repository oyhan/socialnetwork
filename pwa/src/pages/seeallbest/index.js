...
export default function SeeAllBest() {
    const [openSeachBobo, setOpenSeachBobo] = useState(false);

    const [loading, nearRestaurants, error] = useHttpClient("/restaurant/best/0/20", "Get", r => r.response);
    ...
    return (
        <>
            <AppBar back leftIcons={leftIcons} short title={<HeaderTopChip variant='secondary' handleClick={() => { }} title="بهترین رستوران ها" />} />
            <Typography>
                رستوران ها
            </Typography>
            <ThinDivider />
